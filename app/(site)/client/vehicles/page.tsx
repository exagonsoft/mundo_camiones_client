...
const AuctioneerVehicles = () => {
  ...
  const fetchVehicles = async () => {
    try {
      if(session?.user.accessToken){
        const  {data, errors} = await getVehicles(session.user.accessToken.access_token);
        if(!errors){
          setVehicles(data!);
        }
      }
  ...
  return (
    ...
    >
      ...
      <div className="w-full flex flex-col justify-start items-start rounded-lg gap-4">
        {
